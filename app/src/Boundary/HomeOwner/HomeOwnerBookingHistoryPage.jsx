...
function HomeOwnerBookingHistoryPage() {
  ...
  // Search Tab State
  const [searchParams, setSearchParams] = useState({ startDate: '', endDate: '', serviceCategoryId: '' });
  ...
  useEffect(() => {
    if (loggedInUsername) { // Only fetch categories if user is logged in and might search
      fetchCategories();
    }
  ...
  const handleSearchBookings = async (e) => {
    ...
    try {
      ...
      if (searchParams.serviceCategoryId) query.append('serviceCategoryId', searchParams.serviceCategoryId);

      const results = await apiCall(`/api/homeowner/bookings/search?${query.toString()}`);
  ...
  const fetchBookingDetails = useCallback(async (bookingId) => {
    ...
    try {
      ...
      showMessage(error.message || `Failed to fetch details for booking ID ${bookingId}.`);
  ...
  return (
    <div>
      {loggedInUsername && <p style={{ fontStyle: 'italic', textAlign: 'right' }}>Viewing booking history for: {loggedInUsername}</p>}
      ...
      <div style={{ marginBottom: '15px', borderBottom: '1px solid #ccc', paddingBottom: '5px' }}>
        <button onClick={() => { setActiveTab('search'); clearMessages(); setSelectedBookingId(null); setBookingDetails(null); }} disabled={activeTab === 'search'}>Search Bookings</button>
        <button onClick={() => setActiveTab('details')} disabled={activeTab === 'details' || !selectedBookingId}>Booking Details</button>
      </div>
      ...
      {activeTab === 'search' && !isLoading && loggedInUsername && (
        <div>
          <h3>Find Your Past Bookings</h3>
          <form onSubmit={handleSearchBookings}>
            <div>
              <label htmlFor="startDate">Start Date: </label>
              <input type="date" id="startDate" name="startDate" value={searchParams.startDate} onChange={handleSearchInputChange} style={{ marginRight: '10px' }} />
            </div>
            <div>
              <label htmlFor="endDate">End Date: </label>
              <input type="date" id="endDate" name="endDate" value={searchParams.endDate} onChange={handleSearchInputChange} style={{ marginRight: '10px' }} />
            </div>
            <div>
            ...
            <button type="submit" disabled={isLoading || isFetchingCategories}>Search Bookings</button>
          </form>
          <h4>Results:</h4>
          {searchResults.length > 0 ? (
            ...
            !message.text && <p>No booking history results to display. Please try different search filters. (˘･_･˘)</p>
          )}
        </div>
      )}
      {!loggedInUsername && activeTab === 'search' && <p>Please log in to search your booking history.</p>}
      ...
      {activeTab === 'details' && !isFetchingDetails && (
        <div>
          ...
          {bookingDetails ? (
            <div>
              <p><strong>Booking ID:</strong> {selectedBookingId}</p>
              <p><strong>Booking Date:</strong> {bookingDetails.bookingDate}</p>
              <p><strong>Cleaner:</strong> {bookingDetails.cleanerName}</p>
              <p><strong>Service Category:</strong> {bookingDetails.serviceCategoryName}</p>
              <p><strong>Service Description:</strong> {bookingDetails.serviceDescription}</p>
              <button onClick={() => { setActiveTab('search'); clearMessages(); setSelectedBookingId(null); setBookingDetails(null); }}>← Back to Search Results</button>
            </div>
          ) : (
            selectedBookingId && <p>Loading details for booking ID {selectedBookingId}...</p> ||
            <p>Select a booking from the search results to view its details. (*＾▽＾)／</p>
          )}
        </div>
      )}
      {activeTab === 'details' && isFetchingDetails && <p>Fetching booking details...</p>}
    </div>
  );
}
