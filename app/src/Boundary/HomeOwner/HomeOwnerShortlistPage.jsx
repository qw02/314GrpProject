...
function HomeOwnerShortlistPage() {
  ...
  const [searchParams, setSearchParams] = useState({ cleanerUsername: '', description: '', minPrice: '', maxPrice: '' });
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      showMessage('No user session found. Please log in to view your shortlist.', 'error');
  ...
  const handleSearchShortlist = async (e) => {
    ...
    try {
      ...
      if (searchParams.cleanerUsername) query.append('cleanerUsername', searchParams.cleanerUsername);
      ...
      const serviceIds = await apiCall(`/api/homeowner/shortlist/search?${query.toString()}`);
      setSearchResults(serviceIds || []);
      if (!serviceIds || serviceIds.length === 0) {
        showMessage('No shortlisted services found matching your criteria.', 'info');
      }
    } catch (error) {
  ...
  return (
    <div>
      {loggedInUsername && <p style={{ fontStyle: 'italic', textAlign: 'right' }}>Viewing shortlist for: {loggedInUsername}</p>}
      ...
      <div style={{ marginBottom: '15px', borderBottom: '1px solid #ccc', paddingBottom: '5px' }}>
        <button onClick={() => { setActiveTab('welcome'); clearMessages(); }} disabled={activeTab === 'welcome'}>Welcome</button>
        <button onClick={() => { setActiveTab('searchShortlist'); clearMessages(); }} disabled={activeTab === 'searchShortlist'}>Search My Shortlist</button>
        <button onClick={() => setActiveTab('viewDetail')} disabled={activeTab === 'viewDetail' || !selectedService}>View Service Detail</button>
      </div>
      ...
      {activeTab === 'welcome' && (
        <div>
          ...
          <p>Here you can manage and review the services you've shortlisted. ( ´ ▽ ` )ﾉ</p>
          <p>Click on the "Search My Shortlist" tab to find specific services or browse all your saved items.</p>
        </div>
      )}
      ...
      {activeTab === 'searchShortlist' && !isLoading && loggedInUsername && (
        <div>
          <h3>Search Your Shortlisted Services</h3>
          <form onSubmit={handleSearchShortlist}>
            <input
              type="text" name="cleanerUsername" placeholder="Cleaner's username contains..."
              value={searchParams.cleanerUsername} onChange={handleSearchInputChange}
            ...
            />
            <button type="submit" disabled={isLoading}>Search Shortlist</button>
          </form>
          <h4>Results (Service IDs):</h4>
            ...
            <ul>
              {searchResults.map(serviceId => (
                <li key={serviceId}>
                  ...
                  >
                    {isFetchingDetails && selectedService?.serviceId === serviceId ? 'Loading...' : 'View Details'}
                  </button>
                </li>
              ))}
            </ul>
          ) : (
            !message.text && <p>No shortlisted services match your current search, or your shortlist is empty. (˘･_･˘)</p>
          )}
        </div>
      )}
        ...
        <div>
          ...
          {selectedService ? (
            <div>
              ...
              <p><strong>Category:</strong> {selectedService.categoryName || `Category ID: ${selectedService.categoryId}`}</p>
              <p><strong>Description:</strong> {selectedService.description}</p>
              <p><strong>Price Per Hour:</strong> €{typeof selectedService.pricePerHour === 'number' ? selectedService.pricePerHour.toFixed(2) : 'N/A'}</p>
            </div>
          ) : (
            <p>Please select a service ID from the "Search My Shortlist" tab to view its details. (⌒_⌒;)</p>
          )}
        </div>
      )}
      {activeTab === 'viewDetail' && isFetchingDetails && <p>Fetching service details...</p>}
    </div>
  );
}
