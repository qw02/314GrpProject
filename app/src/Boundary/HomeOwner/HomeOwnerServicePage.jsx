...
function HomeOwnerServicePage() {
  ...
  const [isCheckingShortlistStatus, setIsCheckingShortlistStatus] = useState(false);
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      showMessage('No user session found. Please log in to manage shortlists.', 'info');
  ...
  const checkIfShortlisted = useCallback(async (serviceId) => {
    if (!loggedInUsername) return false;
    setIsCheckingShortlistStatus(true);
    clearMessages();
    try {
      const response = await apiCall(`/api/homeowner/shortlist/entry/${loggedInUsername}/${serviceId}`);
      return response.exists || false;
    } catch (error) {
      console.error(`Error checking shortlist status for service ${serviceId}:`, error);
      return false;
    } finally {
  ...
  useEffect(() => {
    if (selectedService && activeTab === 'details' && loggedInUsername) {
      ...
      checkStatus();
    } else if (selectedService && activeTab === 'details' && !loggedInUsername) {
      setIsShortlisted(false); // Can't be shortlisted if not logged in
    }
  }, [selectedService, activeTab, checkIfShortlisted, loggedInUsername, showMessage]);
  ...
  const handleSearchServices = async (e) => {
    ...
    try {
      ...
      // The backend /api/homeowner/services/search does not seem to require homeownerUsername
      // It might accept 'username' for cleaner's username, which we are not using in this simple form.
      const results = await apiCall(`/api/homeowner/services/search?${query.toString()}`);
  ...
  const fetchServiceDetails = async (serviceId) => {
    ...
    try {
      const serviceDetails = await apiCall(`/api/homeowner/service/${serviceId}`);
  ...
  const handleSaveToShortlist = async () => {
    ...
    if (!loggedInUsername) {
      showMessage('You must be logged in to save services to your shortlist.', 'error');
      return;
    }
    ...
    try {
      ...
      showMessage(result.message || 'Service saved to shortlist successfully!', 'success');
  ...
  return (
    <div>
      {loggedInUsername && <p style={{ fontStyle: 'italic', textAlign: 'right' }}>Logged in as: {loggedInUsername}</p>}
      ...
      <div style={{ marginBottom: '15px', borderBottom: '1px solid #ccc', paddingBottom: '5px' }}>
        <button onClick={() => {
          ...
          clearMessages();
        }} disabled={activeTab === 'welcome'}>Welcome
        </button>
        <button onClick={() => {
          setActiveTab('search');
          clearMessages();
        }} disabled={activeTab === 'search'}>Search Services
        </button>
        <button onClick={() => setActiveTab('details')} disabled={activeTab === 'details' || !selectedService}>View
          Service Detail
        </button>
      </div>
      ...
      {/* Feedback Messages */}
      {message.text && (
        <p style={{ color: message.type === 'success' ? 'green' : (message.type === 'info' ? 'darkorange' : 'red') }}>
          {message.text}
        </p>
      )}

      {/* Loading Indicators */}
      {(isLoading || isFetchingDetails || isSavingToShortlist || isCheckingShortlistStatus) && <p>Loading...</p>}
      ...
      {activeTab === 'search' && !isLoading && (
        <div>
          ...
          <form onSubmit={handleSearchServices}>
            <input
              type="text" name="categoryName" placeholder="Category (e.g., Window Cleaning)"
              value={searchParams.categoryName} onChange={handleSearchInputChange}
              style={{ marginRight: '10px', marginBottom: '5px' }}
            />
            <input
              type="text" name="description" placeholder="Description contains..."
              value={searchParams.description} onChange={handleSearchInputChange}
              style={{ marginRight: '10px', marginBottom: '5px' }}
            />
            <input
              type="number" name="minPrice" placeholder="Min Price/hr"
              value={searchParams.minPrice} onChange={handleSearchInputChange}
              min="0" step="0.01" style={{ width: '120px', marginRight: '5px', marginBottom: '5px' }}
            />
            <input
              type="number" name="maxPrice" placeholder="Max Price/hr"
              value={searchParams.maxPrice} onChange={handleSearchInputChange}
              min="0" step="0.01" style={{ width: '120px', marginRight: '10px', marginBottom: '5px' }}
            />
            ...
            <ul>
              {searchResults.map(service => (
                <li key={service.serviceId}>
                  <strong>{service.categoryName || `Category ID: ${service.categoryId}`}</strong> by {service.cleanerUsername}
                  <br/>
                  <em>{service.description.substring(0, 100)}{service.description.length > 100 ? '...' : ''}</em>
                  <br/>
                  Price: €{typeof service.pricePerHour === 'number' ? service.pricePerHour.toFixed(2) : 'N/A'}/hr
                  <button
                    onClick={() => fetchServiceDetails(service.serviceId)}
                    style={{ marginLeft: '10px' }}
                    disabled={isFetchingDetails}
                  >
                    {isFetchingDetails && selectedService?.serviceId === service.serviceId ? 'Loading...' : 'View Details'}
                  </button>
                </li>
              ))}
            </ul>
          ) : (
            !message.text && <p>No results to display. Please refine your search or try different terms.</p>
          )}
        </div>
      )}
        ...
        <div>
          ...
          {selectedService ? (
            <div>
              ...
              <p><strong>Provided by:</strong> {selectedService.cleanerUsername}</p>
              <p><strong>Category:</strong> {selectedService.categoryName}</p>
              <p><strong>Description:</strong> {selectedService.description}</p>
              <p><strong>Price Per
                Hour:</strong> €{typeof selectedService.pricePerHour === 'number' ? selectedService.pricePerHour.toFixed(2) : 'N/A'}
              </p>
              <hr/>
              {loggedInUsername ? (
                isCheckingShortlistStatus ? <p>Checking shortlist status...</p> :
                  <button
                    onClick={handleSaveToShortlist}
                    disabled={isSavingToShortlist || isShortlisted}
                  >
                    {isShortlisted ? '✓ Already in Shortlist' : (isSavingToShortlist ? 'Saving...' : 'Save to Shortlist ♡')}
                  </button>
              ) : (
                <p style={{ color: 'orange' }}>Please log in to save this service to your shortlist.</p>
              )}
            </div>
          ) : (
            <p>Please select a service from the "Search Services" tab to view its details.</p>
          )}
        </div>
      )}
